...
export type InitialBootstrapControlledPreparationReferenceReadStageEvidence =
  | YdbReferenceResolverReadStage
  | 'UNOBSERVED'
  | 'DIAGNOSTIC_FAILED';

export type InitialBootstrapControlledPreparationReconciliationReadStageEvidence =
  | InitialSourceRevisionEvidenceReadStage
  | 'UNOBSERVED'
  | 'DIAGNOSTIC_FAILED';

export type InitialBootstrapControlledPreparationMetadataScanCostEvidence =
  | 'UNOBSERVED'
  | 'LT_10_RU'
  | 'GE_10_LT_3000_RU'
  | 'GE_3000_RU'
...
interface InitialBootstrapControlledPreparationReferenceReadStageTracker {
  observeStage(stage: unknown): void;
  markDiagnosticFailed(): void;
  evidence(): InitialBootstrapControlledPreparationReferenceReadStageEvidence;
}

export function createInitialBootstrapControlledPreparationReferenceReadStageTracker(
): InitialBootstrapControlledPreparationReferenceReadStageTracker {
  let latest: InitialBootstrapControlledPreparationReferenceReadStageEvidence = 'UNOBSERVED';
  let diagnosticFailed = false;

  return Object.freeze({
    observeStage(stage: unknown) {
      try {
        if (
          typeof stage !== 'string'
          || !YDB_REFERENCE_RESOLVER_READ_STAGES.has(stage as YdbReferenceResolverReadStage)
        ) {
          diagnosticFailed = true;
          return;
        }
        latest = stage as YdbReferenceResolverReadStage;
      } catch {
        diagnosticFailed = true;
      }
    },
...
const INITIAL_SOURCE_REVISION_EVIDENCE_READ_STAGES = new Set<InitialSourceRevisionEvidenceReadStage>([
  'REVISION_METADATA_SCAN',
  'REVISION_PAYLOAD_BATCH',
  'REVISION_COLLISION_READ',
]);

interface InitialBootstrapControlledPreparationReconciliationReadStageTracker {
  observeStage(stage: unknown): void;
  markDiagnosticFailed(): void;
  ...
  return Object.freeze({
    observeStage(stage: unknown) {
      try {
        if (
          typeof stage !== 'string'
          || !INITIAL_SOURCE_REVISION_EVIDENCE_READ_STAGES.has(stage as InitialSourceRevisionEvidenceReadStage)
        ) {
          diagnosticFailed = true;
          return;
        }
        latest = stage as InitialSourceRevisionEvidenceReadStage;
      } catch {
        diagnosticFailed = true;
      }
    },
...
const INITIAL_RECOVERY_SERVERLESS_THROTTLING_RU_PER_SECOND = 10 as const;
const INITIAL_RECOVERY_SERVERLESS_BURST_SECONDS = 5 * 60;
const INITIAL_RECOVERY_SERVERLESS_FULL_BURST_RU = (
  INITIAL_RECOVERY_SERVERLESS_THROTTLING_RU_PER_SECOND
  * INITIAL_RECOVERY_SERVERLESS_BURST_SECONDS
);

interface InitialBootstrapControlledPreparationMetadataScanCostTracker {
  observeEstimatedRequestUnits(value: unknown): void;
  markDiagnosticFailed(): void;
  evidence(): InitialBootstrapControlledPreparationMetadataScanCostEvidence;
}

export function createInitialBootstrapControlledPreparationMetadataScanCostTracker(
): InitialBootstrapControlledPreparationMetadataScanCostTracker {
  let latest: InitialBootstrapControlledPreparationMetadataScanCostEvidence = 'UNOBSERVED';
  let diagnosticFailed = false;

  return Object.freeze({
    observeEstimatedRequestUnits(value: unknown) {
      try {
        if (value === null) {
          latest = 'STATS_UNAVAILABLE';
          return;
        }
        ...
        latest = value >= INITIAL_RECOVERY_SERVERLESS_FULL_BURST_RU
          ? 'GE_3000_RU'
          : value >= INITIAL_RECOVERY_SERVERLESS_THROTTLING_RU_PER_SECOND
            ? 'GE_10_LT_3000_RU'
            : 'LT_10_RU';
      } catch {
        diagnosticFailed = true;
...
async function diagnoseStagingControlledPreparation(
  ...
  try {
    ...
    if (referenceReadStageTracker !== null) referenceReadStageEvidence = referenceReadStageTracker.evidence();
