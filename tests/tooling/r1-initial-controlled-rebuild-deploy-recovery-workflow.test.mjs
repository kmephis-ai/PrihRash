...
test('deploy recovery binds exact failed pre-invoke deploy or postflight proof modes and WU7 authority', async () => {
  ...
  assert.match(workflow, /select\(\.name == "Invoke exact controlled rebuild tag synchronously once" and \.conclusion == "skipped"\)/);
...
test('deploy recovery classifies only provider metadata and cannot create or invoke', async () => {
  ...
  assert.match(workflow, /--arg runtime_sa "\$runtime_sa_id"/);
...
test('deploy recovery emits enum-only APPLIED, NOT_APPLIED or RECOVERY_REQUIRED evidence', async () => {
  ...
  assert.match(workflow, /RUNTIME_SA_READ_FAILED/);
  assert.match(workflow, /RUNTIME_SA_METADATA_INVALID/);
  assert.match(workflow, /TAG_RUNTIME_SERVICE_ACCOUNT_NOT_EXACT/);
  assert.doesNotMatch(workflow, /TAG_ASYNC_CONFIG_MISSING/);
