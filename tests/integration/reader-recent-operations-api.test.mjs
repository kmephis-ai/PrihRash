...
const COMMITTED_RUN_ROW = Object.freeze({
  ...
  source_snapshot_digest: 'synthetic-reader-api-verified-shadow',
  state: 'COMMITTED',
  rows_seen: 1n,
  rows_new: 1n,
  rows_changed: 0n,
...
function adapterCapturing(capture, { committed = true } = {}) {
...
test('unknown, duplicate-like, blank and noncanonical values fail with safe request codes', () => {
  ...
  assert.throws(
    () => parseReaderRecentOperationsApiRequest({ accountId: 'private-secret-marker' }),
    (error) => error instanceof ReaderApiRequestError
      && error.message === 'INVALID_QUERY_PARAMETER'
      && !error.message.includes('private-secret-marker'),
  );
});

test('execution delegates through verified-shadow admission and returns versioned JSON-safe envelope', async () => {
  const capture = [];
  ...
  assert.equal(capture[1].parameters.limit.value, 8n);
  assert.equal(capture[1].parameters.type.value, 'INCOME');
  assert.equal(capture[1].parameters.account_id.value, ACCOUNT_ID);
  assert.doesNotThrow(() => JSON.stringify(response));
});

test('request validation fails before any provider read', async () => {
  const capture = [];
...
test('missing verified shadow propagates Reader unavailability before canonical transactions read', async () => {
  const capture = [];
  await assert.rejects(
    () => executeReaderRecentOperationsApiRequest(adapterCapturing(capture, { committed: false }), {}),
