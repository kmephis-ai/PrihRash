...
test('readiness probe performs one Google read and seven read-only YDB checks', async () => {
  ...
  assert.equal(capture.statements.length, 7);
  ...
  assert.equal(capture.statements[6].text, 'SELECT source_record_id, revision, migration_run_id, observed_at, row_hint, CAST(row_digest AS Utf8) AS row_digest, change_class FROM source_record_revisions VIEW idx_source_record_revisions_run_revision LIMIT 0');
...
test('schema migration evidence fails closed for malformed rows', async () => {
  const malformedRows = [
    ...
    [{ version: 1, checksum: '', applied_at: '2026-09-01T00:00:00.000Z' }, validMigrationRows[1], validMigrationRows[2], validMigrationRows[3]],
    [{ version: 1, checksum: ' x ', applied_at: '2026-09-01T00:00:00.000Z' }, validMigrationRows[1], validMigrationRows[2], validMigrationRows[3]],
    [{ version: 1, checksum: 'x', applied_at: 'invalid' }, validMigrationRows[1], validMigrationRows[2], validMigrationRows[3]],
    [validMigrationRows[0], { version: 2, checksum: 'x', applied_at: null }, validMigrationRows[2], validMigrationRows[3]],
    [validMigrationRows[0], validMigrationRows[0], validMigrationRows[1], validMigrationRows[2], validMigrationRows[3]],
  ];
...
test('missing and future migration versions are distinct fail-closed blockers', async () => {
  const missing = adapterForReads([[], [], validMigrationRows.slice(0, 3)]).adapter;
  await assert.rejects(
  ...
  const unexpected = adapterForReads([[], [], [
    ...validMigrationRows,
    { version: 5, checksum: 'checksum-005', applied_at: '2026-09-05T00:00:00.000Z' },
  ]]).adapter;
  await assert.rejects(
...
test('YDB readiness read failures are stage-specific, sanitized, and never open a transaction', async () => {
  const cases = [
    ...
    {
      reads: [[], [], validMigrationRows, [], [], [], new Error('private-ydb-endpoint synthetic-revision-index-missing')],
      expectedCode: 'YDB_SOURCE_RECORD_REVISION_RUN_INDEX_SCHEMA_READ_FAILED',
