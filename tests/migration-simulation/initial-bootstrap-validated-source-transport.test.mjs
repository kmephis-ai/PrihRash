...
const base = { verdict: 'RECOVERY_REQUIRED', reason: 'VALIDATED_CURRENT_EMPTY_STAGING_NONEMPTY' };
test('VALIDATED source evidence remains enum-only and cannot cross lifecycle or surface boundaries', async () => {
  ...
  for (const value of [base, { ...base, validatedSourceEvidence: 'private text' }, { ...base, validatedSourceEvidence: 'AUTHORITATIVE_SNAPSHOT_MATCH', stagingRetirementEvidence: 'STALE_STAGING_CURRENT_STATE_EMPTY' }, { ...base, reason: 'VALIDATED_CURRENT_EMPTY_STAGING_EMPTY', validatedSourceEvidence: 'AUTHORITATIVE_SNAPSHOT_MATCH' }]) {
