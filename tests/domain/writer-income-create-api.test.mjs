...
import {
  WRITER_INCOME_CREATE_API_VERSION,
  executeWriterIncomeCreateApiRequest,
} from '../../dist/writer/incomeCreateApi.js';
...
function references({ missing = false, categoryKind = 'INCOME', error = null } = {}) {
  return {
...
async function expectCode(promise, code) {
  await assert.rejects(promise, (error) => {
    ...
    assert.equal(serialized.includes('Зарплата'), false);
    return true;
  });
}
...
test('Writer API first create exposes only minimal immutable CREATED acknowledgement', async () => {
  ...
  const response = await executeWriterIncomeCreateApiRequest({
...
test('Writer API exact replay exposes the same transaction identity with REPLAY and no second create', async () => {
  ...
  const created = await executeWriterIncomeCreateApiRequest(dependencies, REQUEST);
  const replay = await executeWriterIncomeCreateApiRequest(dependencies, { ...REQUEST });
...
test('Writer API preserves #402 idempotency conflict semantics without replacing the committed create', async () => {
  const store = new MemoryStore();
  const dependencies = {
    references: references(),
    store,
  ...
  await executeWriterIncomeCreateApiRequest(dependencies, REQUEST);
  await expectCode(
    executeWriterIncomeCreateApiRequest(dependencies, { ...REQUEST, amountMinor: REQUEST.amountMinor + 1 }),
...
test('Writer API preserves value-free request/reference/dependency errors from the application contract', async (t) => {
  const cases = [
    ...
    ['wrong category kind', { body: REQUEST, refs: references({ categoryKind: 'EXPENSE' }), code: 'CATEGORY_KIND_INVALID' }],
    ['reference failure', {
      body: REQUEST,
  ...
  for (const [name, input] of cases) {
    await t.test(name, async () => {
      const store = new MemoryStore();
      await expectCode(executeWriterIncomeCreateApiRequest({
...
test('Writer API source is framework/provider-neutral and delegates to the existing idempotent create contract', async () => {
  const source = await readFile(new URL('../../src/writer/incomeCreateApi.ts', import.meta.url), 'utf8');

  assert.match(source, /executeIdempotentIncomeCreate/u);
  assert.match(source, /\.\/idempotentIncomeCreate\.js/u);
