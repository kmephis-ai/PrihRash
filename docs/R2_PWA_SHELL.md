...
Lexical evidence также не нормализуется в browser: operation/entity/financial-period ids должны уже быть canonical lowercase UUID; entity labels и non-null description/note — non-empty trimmed strings; dates — full `YYYY-MM-DD`; non-null `capturedAt` — trimmed parseable timestamp. Это parity с existing backend Reader parser: malformed cache/network value блокируется, а не чинится или повышается до новой точности. Synthetic monthly aggregate поэтому хранит canonical `aggregatePeriodMonth=YYYY-MM-01`, хотя owner-facing presentation показывает только `YYYY-MM`.
...
При смене shell cache version старые `prihrash-shell-*` entries удаляются при activation. Это не затрагивает IndexedDB financial cache. Поскольку production Service Worker использует cache-first для перечисленных `SHELL` assets, любое изменение содержимого precached asset требует новой `prihrash-shell-v*` версии; иначе уже установленная PWA может продолжить отдавать старые bytes. Текущая версия после browser lexical-evidence parity — `prihrash-shell-v22`.
