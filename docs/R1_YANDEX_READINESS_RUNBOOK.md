...
Для R1 recovery blocker `RESOURCE_EXHAUSTED` workflow имеет manual-only input `resource_limits_only=true`. Первоначальный direct-WIF metadata path доказал fail-closed `READ_FAILED`: dedicated GitHub deploy identity намеренно не получает YDB read authority. Следующий runtime-SA `Database.List` path также доказал `READ_FAILED`: database-scoped `ydb.viewer` не используется как основание предполагать folder-level enumeration. Поэтому canonical diagnostic не расширяет IAM и не делает folder discovery.

Resource-only режим выполняет обычные exact-source package/YC CLI/OIDC prerequisites, fail-closed проверяет exact current `main`, private Function, exact WIF invoker binding и `triggers=0`, resolve'ит **только metadata** existing readiness Lockbox secret/current version и создаёт отдельную version с entrypoint `index.resourceLimitsHandler`, tag `r1-ydb-resource-limits`, logging disabled. В эту version server-side inject'ится ровно один existing key `ydb_connection_string`; Google keys не передаются. Handler берёт short-lived IAM token прикреплённого `prihrash-backend` из Yandex Function invocation context и приватно парсит current managed-YDB database path. Промежуточному provider segment не присваивается семантика folder: YDB documentation предупреждает приложения не интерпретировать directory segments как стабильную provider model. Из current three-segment cloud path извлекается только final database ID, после чего выполняется ровно один read-only `Database.Get`. Уже provider response обязан fail-closed подтвердить exact returned database ID и `folderId == PRIHRASH_YC_FOLDER_ID`; только это является folder identity proof. Connection string/path/database ID не возвращаются и не логируются. GitHub deploy identity не получает `ydb.viewer`, Lockbox payload и новые access bindings.

Target resolution fail-closed: malformed/non-`grpcs` connection string, duplicate/missing `database` parameter, malformed current cloud path, malformed provider response, auth/provider failure или mismatch returned database ID/folder публикуются как bounded `READ_FAILED` плюс независимый coarse `failureStage`: `TARGET_CONFIG_INVALID | TRANSPORT_FAILED | UNAUTHORIZED | FORBIDDEN | NOT_FOUND | RATE_LIMITED | PROVIDER_5XX | UNEXPECTED_STATUS | MALFORMED_JSON | IDENTITY_MISMATCH | LIMITS_MALFORMED`; для успешной/не-READ_FAILED классификации используется `NONE`. Serverless resource-limit scalars следуют canonical proto3 semantics provider API: implicit scalar fields с default values могут отсутствовать в ProtoJSON. Поэтому omitted `enableThrottlingRcuLimit` интерпретируется как `false`, а omitted `throttlingRcuLimit` / `provisionedRcuLimit` — как `0`; только физически отсутствующие implicit fields получают эти defaults. Явные `null`, отрицательные, unsafe или non-numeric значения остаются fail-closed `LIMITS_MALFORMED`. Numeric HTTP status, Provider/database IDs, names, endpoint/path, IAM token, connection string и raw provider response не входят в evidence. Для доказанной serverless DB public evidence содержит только `enableThrottlingRcuLimit`, `throttlingRcuLimit` и `provisionedRcuLimit`; для dedicated/unknown эти поля `null`. `Database.List`, YDB data/scheme writes, `Database.Update/Create/Delete/Move`, quota/cap mutation, IAM widening и Google reads этим режимом запрещены. Перед deploy и перед единственным private invoke workflow повторно доказывает exact current `main`; после устранения R1 provider blocker temporary diagnostic handler/tag подлежит retirement.
...
logging: disabled
Lockbox secret: `ydb_connection_string` only (server-side injection)
...
```json
{"googleSource":"READY","ydbSchema":"READY","requiredMigrationVersion":4}
```
...
Только `READINESS_READY` на version, собранной из exact current canonical `main`, является current read-only provider PASS. #302 уже был закрыт историческим successful readiness после schema `001..002`; его не переоткрывают для forward migration 003. После будущего `SCHEMA_UPGRADE_004_READY` новый `READINESS_READY` с `requiredMigrationVersion=4` подтверждает schema-v4 gate; #630 mutation boundary может возобновиться только после retirement temporary migration-004 write authority.
