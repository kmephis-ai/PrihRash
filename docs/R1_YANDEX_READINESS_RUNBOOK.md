...
Target resolution fail-closed: malformed/non-`grpcs` connection string, duplicate/missing `database` parameter, foreign folder, malformed provider response, auth/provider failure или mismatch returned database ID/folder публикуются только как bounded `READ_FAILED`. Provider/database IDs, names, endpoint/path, IAM token, connection string и raw provider response не входят в evidence. Для доказанной serverless DB public evidence содержит только `enableThrottlingRcuLimit`, `throttlingRcuLimit` и `provisionedRcuLimit`; для dedicated/unknown эти поля `null`. `Database.List`, YDB data/scheme writes, `Database.Update/Create/Delete/Move`, quota/cap mutation, IAM widening и Google reads этим режимом запрещены. Перед deploy и перед единственным private invoke workflow повторно доказывает exact current `main`; после устранения R1 provider blocker temporary diagnostic handler/tag подлежит retirement.
