...
| B | Exact marker-only VALIDATED → FAILED с fixed error code и unknown-outcome recovery | TRANSACTION + MARKER-OUTCOME PRIMITIVES РЕАЛИЗОВАНЫ SYNTHETIC-ONLY: exact FAILED + empty current = APPLIED; unchanged exact VALIDATED = RECOVERY_REQUIRED/NO_RETRY; historical staging-preservation integration и production workflow ещё не реализованы, live dispatch запрещён |
| C | Fresh bootstrap с новыми identities при сохранении старых audit/staging tables | НЕ РАЗРЕШЁН до отдельного gate после B |

После canonicalization Owner risk exception condition 6 больше не является blocker **только** для exact `35469651936`. Следующий bounded repository step — production Gate B surface, который должен сам получить shared `r1-initial-bootstrap-writer` lock и внутри него выполнить fresh exact-main readiness/recovery/preflight, заново доказать historical correlation + exact `NOT_APPLIED` + остальные Gate A predicates, затем при их точном совпадении выполнить единственную marker-only transaction и terminal read-back. До такого exact-head implementation/CI и fresh in-lock proof live marker mutation запрещена. Existing STAGING retirement workflow не принимает VALIDATED как alias. Cleanup старого evidence, увеличение cap, promotion старого candidate и Gate C не входят в это Owner decision.
