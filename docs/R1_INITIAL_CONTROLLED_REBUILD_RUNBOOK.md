...
`APPLIED` proves only the Function-version deployment layer. It does not prove that the controlled rebuild handler ran, that YDB staging/swap/marker mutations occurred, or that a verified baseline exists. `NOT_APPLIED` permits a later separately gated deploy retry only after its cause is understood and the exact-main/recovery/readiness gates are refreshed as required. `RECOVERY_REQUIRED` forbids invocation and blind replay.

The sole bounded provider-repair case is `RECOVERY_REQUIRED / TAG_ASYNC_CONFIG_MISSING`: when fresh exact-main YDB recovery still proves `VALIDATED_CURRENT_EMPTY_STAGING_ABSENT` and fresh readiness proves `READINESS_READY`, the existing controlled workflow may enter provider repair-only mode. That mode creates a new exact-source Function version with the canonical async configuration, re-proves the private/trigger-free exact tag metadata, emits only `INITIAL_CONTROLLED_REBUILD_PROVIDER_REPAIRED`, and **does not invoke** the controlled handler. After repair, read-only deploy recovery must run again; only a new `APPLIED / EXACT_ACTIVE_TAG` result may be used by the later recovery-mode invocation. If repair deployment or its postflight is uncertain, the same read-only deploy-recovery workflow classifies that pre-invoke outcome before any retry.

The deploy-recovery workflow has no Function-version create/invoke command and no Google/YDB mutation authority. Its published artifacts are bounded enum-only evidence. When a repair attempt fails before invocation and deploy recovery still reports `RECOVERY_REQUIRED / TAG_ASYNC_CONFIG_MISSING`, the same read-only workflow also emits `INITIAL_CONTROLLED_REBUILD_PROVIDER_DIAGNOSTIC`: it independently classifies the exact Function `functions.functionInvoker` binding for the GitHub WIF caller and configured runtime async-invoker service account, the exact Function `functions.editor` binding required for the GitHub WIF caller to create a Function version, observes the exact-Function `functions.viewer` binding without making it a repair prerequisite, and classifies any matching Function operation from the failed GitHub-run time window by safe provider error class only. Raw provider messages, operation IDs, service-account IDs and Function IDs are never published. A further repair attempt remains forbidden while the WIF editor prerequisite is missing/ambiguous/read-failed, while the async-invoker IAM prerequisite is not proven, or until a separate Owner/provider-admin decision authorizes any required IAM mutation.

If a separately authorized later provider repair is required after this ambiguity boundary, the controlled workflow must preserve the exact Function-version create failure as enum-only `INITIAL_CONTROLLED_REBUILD_PROVIDER_DEPLOY_FAILED / failureClass / permissionBoundary`. Allowed failure classes remain bounded to IAM/service-account, request-validation, quota/authentication, source-package, transport and `OTHER` categories. For `PERMISSION_DENIED` only, the runner privately correlates raw stderr against already-known masked resource IDs and safe resource-type phrases, then publishes one bounded boundary enum: exact Function, runtime/async service account, WIF service account, access policy, unresolved service-account/function type, or unresolved. Non-permission failures publish `NOT_APPLICABLE`. Raw CLI stderr is never printed or uploaded. A failed create still requires the normal read-only deploy recovery before any later provider write; the captured class is diagnostic evidence only and never authorizes an automatic retry. Deploy recovery remains backward-compatible with historical three-field failure artifacts.
...
The live WU7 Function may legitimately need longer than a synchronous HTTPS client connection remains reliable. Controlled continuation therefore uses Yandex Cloud Functions asynchronous invocation (`integration=async`) after all exact-main/recovery/readiness/provider-boundary gates pass. The deployed Function version must explicitly enable async invocation with zero provider retries and the existing runtime service account `prihrash-initial-bootstrap` as the async invoker identity; no success/failure YMQ target is configured. The GitHub WIF identity remains the deployment/submission caller only.

### Async invoker IAM prerequisite

WU7 uses `prihrash-initial-bootstrap` as both the Function runtime service account and the private asynchronous invocation service account. This removes the previous dependency on the deployment WIF account using itself as `asyncInvocationConfig.serviceAccountId`.

Before any provider repair under this contract, the exact Function `prihrash-r1-initial-bootstrap` must contain the private bindings required by WU7 deployment and transport:

- GitHub WIF caller `prihrash-github-initial-bootstrap` → `functions.editor` on the exact Function, for Function-version creation;
- GitHub WIF caller `prihrash-github-initial-bootstrap` → `functions.functionInvoker` on the exact Function, for the explicit submission request;
- runtime/async account `prihrash-initial-bootstrap` → `functions.functionInvoker` on the exact Function, because the core Cloud Functions async-invocation contract requires the async service account to invoke the Function.

Yandex Monitoring documentation separately uses `functions.viewer` together with `functions.functionInvoker` for Monitoring→Function notification channels. WU7 does not use Monitoring as the caller, and adding exact-Function `functions.viewer` did not remove the create-time `PERMISSION_DENIED`. Therefore the viewer binding is retained as live provider state but is not a WU7 repair prerequisite. Do not remove it automatically; retirement is a separate least-privilege action after provider diagnosis.

The deployment WIF already requires `iam.serviceAccounts.user` on `prihrash-initial-bootstrap` in order to attach that runtime service account to Function versions; the same exact service-account resource is used for async configuration. The target WU7 contract no longer requires a WIF self-binding. Any historical WIF self-binding is outside this item and must not be removed automatically; retirement requires separate evidence/authority.

Do **not** grant invocation roles on the folder/cloud and do not broaden the deployment WIF to primitive `editor`/`admin` or broader IAM roles as a diagnostic shortcut. Repository workflows do not create or modify IAM bindings. If either required exact-Function `functions.functionInvoker` binding is missing, controlled repair remains fail-closed. After any provider-admin IAM change, refresh exact-main deploy recovery, YDB recovery and readiness before a separately authorized provider repair.
