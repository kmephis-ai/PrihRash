...
Before any provider repair under this contract, the exact Function `prihrash-r1-initial-bootstrap` must contain the complete private async-access set:
...
- runtime/async account `prihrash-initial-bootstrap` → both `functions.functionInvoker` and `functions.viewer` on the exact Function. Yandex documents both roles for the service account selected for asynchronous Function invocation; without the viewer role, authorization may fall back to anonymous and fail.

The deployment WIF already requires `iam.serviceAccounts.user` on `prihrash-initial-bootstrap` in order to attach that runtime service account to Function versions; the same exact service-account resource is now used for async configuration. The target WU7 contract no longer requires a WIF self-binding. Any historical WIF self-binding is outside this item and must not be removed automatically; retirement requires separate evidence/authority.

Do **not** grant these roles on the folder/cloud. Keep `functions.functionInvoker` and `functions.viewer` on the exact Function only; do not replace them with primitive `editor`/`admin` or broader roles. Repository workflows do not create or modify IAM bindings. If any required exact-Function binding is missing, controlled repair remains fail-closed. After provider-admin establishes the runtime/async invoker binding, refresh exact-main deploy recovery, YDB recovery and readiness before any separately authorized provider repair.
