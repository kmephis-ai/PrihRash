...
The only allowed next action is the manual read-only `R1 initial controlled rebuild deploy recovery` workflow on exact current `main`. It must bind both the exact current recovery-code SHA and the separate immutable SHA of the failed controlled-rebuild run, prove that the run failed before invocation, re-check the open WU7 authority boundary and verify that no competing R1 writer is active. The accepted pre-invoke failure modes are: (1) Function-version deploy failed and the controlled invoke step was skipped; or (2) recovery-mode deploy was intentionally skipped, the private/trigger-free live metadata postflight failed, and the controlled invoke step was skipped. The failed-run SHA is intentionally separate because recovery tooling may be merged after the failed provider attempt.
