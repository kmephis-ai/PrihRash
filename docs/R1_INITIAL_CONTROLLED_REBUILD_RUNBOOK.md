...
Before any provider repair under this contract, the exact Function `prihrash-r1-initial-bootstrap` must contain both private invoker bindings:
...
- runtime/async account `prihrash-initial-bootstrap` → `functions.functionInvoker` on the exact Function, for asynchronous execution.
...
Do **not** grant either invoker role on the folder/cloud. Do not replace them with primitive `editor`/`admin` or broader roles. Repository workflows do not create or modify IAM bindings. If either exact Function invoker binding is missing, controlled repair remains fail-closed. After provider-admin establishes the runtime/async invoker binding, refresh exact-main deploy recovery, YDB recovery and readiness before any separately authorized provider repair.
