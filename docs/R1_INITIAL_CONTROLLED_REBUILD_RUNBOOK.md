...
The only allowed next action is the manual read-only `R1 initial controlled rebuild deploy recovery` workflow on exact current `main`. It must bind both the exact current recovery-code SHA and the separate immutable SHA of the failed controlled-rebuild run, prove that the run failed before invocation, re-check the open WU7 authority boundary and verify that no competing R1 writer is active. The accepted pre-invoke failure modes are: (1) Function-version deploy failed and the controlled invoke step was skipped; or (2) the private/trigger-free live metadata postflight failed and the controlled invoke step was skipped, with the deploy step either intentionally skipped in reuse mode or already successful in a deploy/repair mode. The failed-run SHA is intentionally separate because recovery tooling may be merged after the failed provider attempt.
...
The deploy-recovery workflow has no Function-version create/invoke command and no Google/YDB mutation authority. Its published artifacts are bounded enum-only evidence. When a repair attempt fails before invocation and deploy recovery still reports `RECOVERY_REQUIRED / TAG_ASYNC_CONFIG_MISSING`, the same read-only workflow also emits `INITIAL_CONTROLLED_REBUILD_PROVIDER_DIAGNOSTIC`: it proves or rejects the exact Function `functions.functionInvoker` binding for the WIF service account, attempts to prove the exact self-binding `iam.serviceAccounts.user` without changing IAM, and classifies any matching Function operation from the failed GitHub-run time window by safe provider error class only. Raw provider messages, operation IDs, service-account IDs and Function IDs are never published. A third repair attempt is forbidden until this diagnostic localizes a bounded cause or a separate Owner/provider-admin decision resolves the remaining provider ambiguity.
