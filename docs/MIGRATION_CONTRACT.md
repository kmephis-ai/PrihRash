...
7. Existing single-writer boundary должен исключать новый competing dispatch от final preflight до terminalization read-back; workflow lock только собственного recovery workflow недостаточен. Shared execution layer использует общий GitHub Actions concurrency group `r1-initial-bootstrap-writer` для standalone recovery, initial bootstrap, controlled rebuild и swap-recovery surface с `cancel-in-progress=false`. Queue сама по себе не считается полным proof: обычный initial bootstrap дополнительно обязан после получения lock read-only проверить отсутствие exact terminalized marker `FAILED / INITIAL_BOOTSTRAP_STALE_VALIDATED_SNAPSHOT`; если marker существует, он возвращает `RECOVERY_REQUIRED / STALE_VALIDATED_TERMINALIZATION_REQUIRES_GATE_C` до нового claim. Controlled rebuild после terminalization обязан остановиться на отсутствии STAGING/VALIDATED continuation run. Будущий Gate B surface обязан держать тот же shared lock непрерывно от собственного fresh Gate A preflight через marker transaction до post-write read-back; отдельный завершившийся read-only recovery run не доказывает эту непрерывность. Нужны regression proof этих свойств без расширения IAM или включения scheduled writers. Если exclusion нарушен или не доказан — STOP.
