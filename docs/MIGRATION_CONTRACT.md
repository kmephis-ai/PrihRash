...
5. resume разрешён только для того же `STAGING/VALIDATED` run и только при exact совпадении manifest с тем же immutable source observation evidence; mismatch, malformed/duplicate binding или contradictory provider evidence переводят выполнение в recovery-required boundary без current-state mutation;
   - если durable run уже `VALIDATED`, этот lifecycle marker является доказательством, что full pre-promotion reconciliation для exact claimed observation уже прошла; controlled continuation не обязана повторно сканировать и reparsing-ить все durable revision payloads только ради повторного доказательства того же перехода;
   - такой fast-path разрешён только после exact admission + manifest/snapshot/digest resume proof и повторной проверки run/projection invariants; `STAGING` path по-прежнему обязан выполнить full reconciliation перед `VALIDATED`, а любой identity/projection/counter mismatch fail-closed до staging/current mutation;
...
Contiguous durable revision-1 prefix доказывает только exact границу между уже materialized prefix и ещё отсутствующим suffix canonical manifest. Он **не** восстанавливает original revision-evidence batch partition или конкретный failing query: initial planner делит writes по calibrated `estimatedParameterBytes` boundary, а identity manifest намеренно не хранит `raw_payload`/его byte size. Exact initial INSERT batch передаётся одним typed `List<Struct>` parameter через `AS_TABLE($rows)`, поэтому YQL text не растёт вместе с количеством строк и отдельный statement-count cap не создаётся. Manifest-only recovery всё равно не имеет права выводить batch index/end, реконструировать missing write payload или replay'ить suffix как будто original batch известен. Для текущего stale run это causal evidence limit, а не разрешение нового write.
...
#### Stale initial `VALIDATED` recovery с сохранением audit/staging evidence

Owner 2026-09-20 разрешил отдельный recovery contract для stale initial `VALIDATED` с сохранением audit/staging evidence. Этот раздел задаёт условия будущей реализации; сам merge документа не разрешает production invoke. Existing stale-STAGING retirement, ordinary resume и autocontinue не получают нового разрешения. До реализации и доказательства всех gates ниже результат остаётся `RECOVERY_REQUIRED`.

Scope ограничен initial shadow без `COMMITTED` baseline: один stale `VALIDATED` run, exact empty canonical current и существующая непустая run-scoped staging pair. Другие состояния не входят в этот contract. Google остаётся authoritative.

**Gate A — read-only доказательство безопасной terminalization.** Все условия обязательны одновременно:

1. Exact protected current `main`, canonical CI PASS, применимый открытый tracking Issue с Owner decision, fresh readiness и recovery; evidence связано с одним exact run и неизменными source/snapshot/manifest/candidate identities. Evidence другого SHA или run не переносится автоматически.
2. Durable admission доказывает отсутствие `COMMITTED` и ровно один incomplete run в `VALIDATED`, с `finished_at=null` и `error_code=null`. Run ↔ manifest ↔ snapshot metadata согласованы; immutable revisions принадлежат тому же run и exact identities, без missing/duplicate/foreign/ambiguous evidence.
3. Fresh authoritative Google observation имеет другой digest; согласованный read-only classifier подтверждает `AUTHORITATIVE_SNAPSHOT_DIGEST_MISMATCH`. Один phase marker, ошибка чтения, `VALIDATED_METADATA_INVALID` или `VALIDATED_SOURCE_DIAGNOSTIC_FAILED` не заменяют этот proof. Prefix/insertion-only enums не входят в этот gate и не разрешают переиспользование старых identities.
4. Старый validated candidate обычно восстанавливается исключительно из доказанных durable snapshot/manifest/revision evidence и доказанного исходного projection/reference context через existing financial engine. Fresh Google rows нельзя подставлять в старый snapshot или связывать с ним заменой digest. Owner 2026-09-20 отдельно разрешил более слабую альтернативу: privacy-safe temporal correlation исторического invocation должна доказать порядок `swap discrimination reached → invocation completed` без marker мутации, а отдельный exact read-only discriminator должен вернуть `NOT_APPLIED`. Такая комбинация обозначается только enum `TEMPORALLY_CORROBORATED_WITH_EXACT_NOT_APPLIED`; timestamp values, raw logs, provider IDs и payloads не публикуются. Она не доказывает full historical projection/reference equivalence и поэтому является сознательно более слабым доказательством, применимым только к marker-only terminalization этого одного stale run. При отсутствии либо temporal correlation, либо exact `NOT_APPLIED` — `RECOVERY_REQUIRED`. Reconstruction служит только recovery proof, не разрешает promotion старого candidate.
5. Existing read-only swap discriminator доказывает exact `NOT_APPLIED`: canonical current exact empty, обе exact run-scoped staging tables присутствуют и полностью согласованы с candidate при reconstructed proof либо с exact durable staging pair при разрешённой temporal alternative. Structural `VALIDATED_CURRENT_EMPTY_STAGING_NONEMPTY` не заменяет staging reconciliation. `APPLIED`, partial/mismatched staging, mixed/foreign structure или ambiguous outcome запрещают terminalization по этому пути.
6. Исключены активные, queued и поздние writes от всех bootstrap/controlled/recovery writers. Нужны доказанное завершение старых invocations и отсутствие ещё исполняющихся provider mutations, включая ранее отправленный scheme request. GitHub job completion, HTTP timeout, истёкший Function timeout или два одинаковых чтения по отдельности не доказывают завершение server-side scheme operation. Unknown operation outcome остаётся blocker до read-only proof её завершения и exact `NOT_APPLIED`. Если provider не позволяет это доказать доступным evidence, gate не считается пройденным.

Historical one-run Owner exception (2026-09-20): для exact run `35469651936` на source SHA `acb40fb81befc548939f72146df729d27d591cbe` Owner явно принял остаточный риск отсутствия provider-side proof завершения/отсутствия старой SchemeShard `RenameTables` transaction. Только для marker-only Gate B terminalization **этого** stale `VALIDATED` run condition 6 разрешено считать owner-accepted risk exception, если fresh exact-main Gate B invocation под тем же shared writer lock заново докажет все остальные Gate A predicates, включая exact `NOT_APPLIED`, неизменный lifecycle/metadata/candidate state и отсутствие competing writer. Это exception не является доказательством provider completion, не переопределяет unknown outcome как success/failure и не разрешает swap replay, cleanup, promotion, Gate C или другой provider write. Любой fresh `APPLIED`, mixed/ambiguous topology, active/competing writer или изменившийся durable state возвращает `RECOVERY_REQUIRED`. Для всех будущих ambiguous provider mutations condition 6 остаётся строгим и требует durable provider-completion evidence.
7. Existing single-writer boundary должен исключать новый competing dispatch от final preflight до terminalization read-back; workflow lock только собственного recovery workflow недостаточен. Shared execution layer использует общий GitHub Actions concurrency group `r1-initial-bootstrap-writer` для standalone recovery, initial bootstrap, controlled rebuild, swap-recovery и dedicated Gate B surface `r1-initial-bootstrap-stale-validated-terminalization.yml` с `cancel-in-progress=false`. Queue сама по себе не считается полным proof: обычный initial bootstrap дополнительно обязан после получения lock read-only проверить отсутствие exact terminalized marker `FAILED / INITIAL_BOOTSTRAP_STALE_VALIDATED_SNAPSHOT`; если marker существует, он возвращает `RECOVERY_REQUIRED / STALE_VALIDATED_TERMINALIZATION_REQUIRES_GATE_C` до нового claim. Controlled rebuild после terminalization обязан остановиться на отсутствии STAGING/VALIDATED continuation run. Gate B surface обязан держать тот же shared lock непрерывно от собственного fresh Gate A preflight через marker transaction до post-write read-back; unknown marker outcome разрешает только read-only durable recovery и не разрешает второй WRITE. Отдельный завершившийся read-only recovery run или само наличие workflow не доказывает эту непрерывность: proof возникает только из exact live Gate B run. Нужны regression proof этих свойств без расширения IAM или включения scheduled writers. Если exclusion нарушен или не доказан — STOP.

Gate A должен выдавать только bounded privacy-safe classification. Raw source rows/payloads, digests, private identities и reconciliation totals остаются private. Изменение run/source/provider state после proof инвалидирует его и требует read-only reconciliation; это не повод повторить write.

**Gate B — отдельно разрешённая marker-only terminalization.** Только после реализованного Gate A, synthetic verification, PR/CI и отдельного exact-main production gate допустима одна serializable transaction:

- внутри transaction повторно проверить admission и exact empty canonical current; optimistic predicates проверяют exact `VALIDATED` row целиком: identity, lifecycle fields, source digest и все counters;
- изменить только `state=FAILED`, `finished_at` и фиксированный `error_code=INITIAL_BOOTSTRAP_STALE_VALIDATED_SNAPSHOT` этой `migration_runs` row; другие поля не менять;
- transactional read-back обязан exact-сверить expected terminal row; mismatch откатывает transaction. После commit read-only proof повторно подтверждает exact terminal row, unchanged current и сохранность старых audit/staging evidence;
- не выполнять scheme operations, staging/current/reference writes, `copyTables`, `renameTables`, removal, cleanup или новый bootstrap в этой invocation. Новый error code является частью recovery contract, а не разрешением обойти существующие lifecycle guards.

Если transaction outcome неизвестен, допускается только read-only recovery: exact expected `FAILED` marker с сохранёнными invariants доказывает завершённую terminalization; unchanged exact `VALIDATED` означает, что completion не доказан и automatic retry запрещён; любое другое состояние остаётся `RECOVERY_REQUIRED`. Ни failure code, ни `FAILED` marker сами по себе не доказывают swap outcome или verified baseline. Новая writer attempt после unknown outcome требует завершения всех возможных in-flight операций и заново доказанного gate.

**Gate C — новый bootstrap отдельно.** После доказанной terminalization новый initial claim разрешается только отдельным gate с fresh exact-main readiness/recovery и current authoritative observation. Новый run/snapshot/manifest и row identities выделяются заново; старые identities не входят в candidate pool. Run-scoped staging path должен быть новым. Старые `source_snapshots`, `initial_bootstrap_identity_manifests`, append-only `source_record_revisions` и обе старые staging tables сохраняются неизменными, включая содержимое; их нельзя удалить, truncate, перезаписать или считать target следующего rebuild. Ожидаемое сохранение старого audit evidence не должно становиться причиной обхода foreign-state guards: допустимое сосуществование проверяется отдельно exact по terminalized run.

Перед новым bootstrap проверяется, что сохранение старого evidence и нового candidate укладывается в существующие resource caps и cost policy; contract не разрешает увеличение cap/quota, новую DDL/schema, IAM widening или cleanup ради вместимости. Новый run проходит весь обычный claim/validation/staging/swap/COMMITTED proof. До этого verified baseline отсутствует, Reader fail-closed; Google mutation, timer/scheduled sync, cutover и production Writer не разрешены.

Acceptance будущего implementation unit: synthetic fixtures должны доказать безопасную terminalization и отказ при source/metadata/candidate mismatch, nonempty current, multiple runs, changed lifecycle/counters, incomplete staging, `APPLIED`/unknown swap, active/late writer, competing dispatch и unknown marker outcome. Отдельно проверяются неизменность всех audit/staging tables, отсутствие identity reuse и отсутствие bootstrap в terminalization invocation. Полный `npm run check` и canonical CI обязательны. Authority этого recovery path retires после доказанного нового `COMMITTED` baseline; сохранённые audit/staging evidence автоматически не удаляются и после retirement.
