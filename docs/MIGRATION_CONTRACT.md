...
7. Existing single-writer boundary должен исключать новый competing dispatch от final preflight до terminalization read-back; workflow lock только собственного recovery workflow недостаточен. Нельзя предполагать наличие такого межпроцессного exclusion в текущей реализации. Нужны проверяемый механизм и regression proof без расширения IAM или включения scheduled writers. Если exclusion нарушен или не доказан — STOP.
