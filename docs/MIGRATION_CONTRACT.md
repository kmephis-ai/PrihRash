...
4. Старый validated candidate восстанавливается исключительно из доказанных durable snapshot/manifest/revision evidence и доказанного исходного projection/reference context через существующий financial engine. Fresh Google rows нельзя подставлять в старый snapshot или связывать с ним заменой digest. Текущее reference/historical evidence допустимо только если его соответствие исходному context доказано; при невозможности восстановления — `RECOVERY_REQUIRED`. Reconstruction служит только recovery proof, не разрешает promotion старого candidate.
5. Existing read-only swap discriminator доказывает exact `NOT_APPLIED` для того же исторического candidate: canonical current exact empty, обе exact run-scoped staging tables присутствуют и полностью согласованы с candidate. Structural `VALIDATED_CURRENT_EMPTY_STAGING_NONEMPTY` не заменяет staging reconciliation. `APPLIED`, partial/mismatched staging, mixed/foreign structure или ambiguous outcome запрещают terminalization по этому пути.
