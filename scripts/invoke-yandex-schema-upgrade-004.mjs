...
const UPGRADE_TAG = 'r1-schema-upgrade-004';
const MAX_CAPTURE_BYTES = 64 * 1024;
const INVOKE_TIMEOUT_MS = 90_000;
const SAFE_READY = Object.freeze({
  ydbSchema: 'READY',
  appliedMigrationVersions: [1, 2, 3, 4],
});
const SAFE_PASS = Object.freeze({ status: 'PASS', code: 'SCHEMA_UPGRADE_004_READY' });
const SAFE_CONFIG_FAILURE = Object.freeze({ status: 'FAIL', code: 'SCHEMA_UPGRADE_004_CONFIG_INVALID' });
const SAFE_INVOKE_FAILURE = Object.freeze({ status: 'FAIL', code: 'SCHEMA_UPGRADE_004_INVOKE_FAILED' });
const SAFE_INVOKE_OUTPUT_INVALID = Object.freeze({ status: 'FAIL', code: 'SCHEMA_UPGRADE_004_INVOKE_OUTPUT_INVALID' });
const SAFE_INVOKE_NONZERO_UNCLASSIFIED = Object.freeze({ status: 'FAIL', code: 'SCHEMA_UPGRADE_004_INVOKE_NONZERO_UNCLASSIFIED' });
const SAFE_INVOKE_MARKER_AMBIGUOUS = Object.freeze({ status: 'FAIL', code: 'SCHEMA_UPGRADE_004_INVOKE_MARKER_AMBIGUOUS' });
...
async function invokeUpgrade(environment = process.env) {
  const functionId = environment.PRIHRASH_YANDEX_SCHEMA_UPGRADE_004_FUNCTION_ID;
