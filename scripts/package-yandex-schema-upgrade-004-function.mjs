...
const dependencies = Object.fromEntries(dependencyNames.map((name) => {
  ...
  if (typeof version !== 'string' || version.length === 0) {
    throw new Error(`YANDEX_SCHEMA_UPGRADE_004_PACKAGE_INVALID: missing root dependency ${name}`);
  }
  return [name, version];
}));
const runtimePackage = {
  name: 'prihrash-yandex-schema-upgrade-004-function',
...
const distFiles = [
  'runtime/yandexCloudSchemaUpgrade004Function.js',
  'runtime/ydbSchemaUpgrade004.js',
...
await cp(
  resolve(ROOT, 'db', 'migrations', '004_source_record_revision_run_index.sql'),
  resolve(ARTIFACT_ROOT, 'migrations', '004_source_record_revision_run_index.sql'),
);
await writeFile(
  resolve(ARTIFACT_ROOT, 'index.js'),
  "export { schemaUpgrade004Handler } from './dist/runtime/yandexCloudSchemaUpgrade004Function.js';\n",
  'utf8',
);
await writeFile(
