...
const DIAGNOSTIC_TAG = 'r1-initial-bootstrap-recovery-diagnostic';
const MAX_CAPTURE_BYTES = 16 * 1024;
const INVOKE_TIMEOUT_MS = 60_000;

const SAFE_CONFIG_FAILURE = Object.freeze({
  status: 'FAIL',
  code: 'INITIAL_BOOTSTRAP_RECOVERY_DIAGNOSTIC_INVOKER_CONFIG_INVALID',
});
const SAFE_INVOKE_FAILURE = Object.freeze({
  status: 'FAIL',
  code: 'INITIAL_BOOTSTRAP_RECOVERY_DIAGNOSTIC_INVOKE_FAILED',
});
const SAFE_OUTPUT_FAILURE = Object.freeze({
  status: 'FAIL',
  code: 'INITIAL_BOOTSTRAP_RECOVERY_DIAGNOSTIC_INVOKE_OUTPUT_INVALID',
});
...
function parseExactResult(stdout) {
  ...
  if (
    result.status === 'PASS'
    && result.code === 'INITIAL_BOOTSTRAP_RECOVERY_DIAGNOSTIC_CLASSIFIED'
...
async function invokeRecoveryDiagnostic(environment = process.env) {
  ...
  try {
    const { stdout } = await execFileAsync(
      ycBinary,
      ['serverless', 'function', 'invoke', '--id', functionId, '--tag', DIAGNOSTIC_TAG, '--retry', '0', '--no-user-output'],
      {
        encoding: 'utf8',
...
const result = await invokeRecoveryDiagnostic();
